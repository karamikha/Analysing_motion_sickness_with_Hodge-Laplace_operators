...
for el in ["BCV", "tACS"]:
    ...
    for k in (0, 1):
        ...
        vectors_of_eigenvalues_for_active = FindVectorsOfEigenvalues(el, "Active", k)
        vectors_of_eigenvalues_for_sham = FindVectorsOfEigenvalues(el, "Sham", k)
        ...
        _, p_value = sts.mannwhitneyu(vectors_of_eigenvalues_medians_for_active, vectors_of_eigenvalues_medians_for_sham, alternative="two-sided")
        alpha = 0.05
        if p_value < 0.05:
            print(f"Spectrа of eigenvalues for active and sham treatment have differences, p-value = {p_value}")
        else:
            print(f"Spectrа of eigenvalues for active and sham treatment are interrelated, p-value = {p_value}")
